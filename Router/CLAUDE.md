...
2. Модератор проверяет сообщение: блокировка / напоминание / ОК
3. Если ОК — Роутер запускает stdio-bridge (если не запущен для этой сессии)
4. Передаёт вопрос через HTTP POST /send → stdio-bridge → Claude
5. Стримит ответ Claude обратно через ЕХТ_Центрифуга.Publish в канал сессии
