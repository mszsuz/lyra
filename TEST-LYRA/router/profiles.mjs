...
export function renderSystemPrompt(template, session, profile) {
  ...
  if (session.configName) {
    ...
    if (!configDir) return '';
    const dd = process.env.LYRA_DATA_DIR || __dirname;
    const globalPath = resolve(dd, 'memory', configDir, 'registry.md');
    ...
    if (session.userId) {
      const userPath = resolve(dd, 'users', session.userId, 'memory', configDir, 'registry.md');
...
export function renderReminder(template, session) {
  ...
  let result = template.replace(/\{\{\s*([\p{L}\w]+)\s*\}\}/gu, (_, name) => vars[name] ?? '');
  // Remove {% Если %} blocks (reminder doesn't support them) — just strip tags
...
export function writeTempFiles(session, profile, toolsPort, config) {
  const tmpDir = resolve(process.env.LYRA_DATA_DIR || __dirname, 'var', 'lobby', session.sessionId);
