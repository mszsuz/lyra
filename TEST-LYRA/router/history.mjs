...
function ensureSessionDir(session) {
  if (!session._historyDir) {
    ...
    const base = session.userId
      ? resolve(dd, 'users', session.userId, session.sessionId)
      : resolve(dd, 'var', 'lobby', session.sessionId);
...
export function moveSessionToUser(session) {
  ...
  const userDir = resolve(dd, 'users', session.userId, session.sessionId);

  // Skip if already in users/
  if (currentDir === userDir) return;

  try {
    mkdirSync(resolve(dd, 'users', session.userId), { recursive: true });
