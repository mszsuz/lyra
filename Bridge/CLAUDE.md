# 1C-Claude Bridge v2 (прототип)

Общайся с пользователем на русском языке.

> **Статус:** прототип. Функции Bridge переходят в Router (Centrifugo + ЕХТ_Лира_Роутер). Код сохраняется как референс и для локальной разработки/отладки.
