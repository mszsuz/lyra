...
**Статус:** Рабочий прототип. bridge.js v2 + Lyra-Chat.epf — полный цикл (2026-02-16)
...
## 6. UI на стороне 1С — Lyra-Chat.epf (РЕАЛИЗОВАНО)

### Что сделано (2026-02-16)

Создана внешняя обработка **Lyra-Chat.epf** (`Chat/src/Lyra-Chat/`), объединяющая:
- HTML-чат на базе фреймворка ЕХТ_Чат (Markdown, подсветка BSL, темы светлая/тёмная, аватары)
- WebSocket-клиент к bridge.js v2 (порт 3003)
- Обработку протокола v2 (session, stream_event, assistant, result, error, mcp_request, claude_exit)
- Выполнение MCP-инструментов на сервере 1С (1c_query, 1c_eval, 1c_metadata, 1c_exec)
- Поле ввода + кнопка отправки + busy-блокировка

### Технические решения
- Реквизиты формы вместо Перем (Перем не работает в модуле формы EPF)
- Маркерный подход [TPL:Шаблон] для программных сообщений (ассистент, ошибки, предупреждения)
- Строковый парсинг JSON через СтрНайти (обход ограничения: ключи с дефисами невалидны для Структуры)
- Дельты накапливаются, финальный result заменяет весь текст (стриминг "по буквам" в TODO)

### Что осталось
- [ ] Стриминг "по буквам" (обновление пузыря чата в реальном времени вместо показа только финального result)
- [ ] Индикатор "Лира печатает..."
- [ ] Индикатор "выполняет запрос к базе..." при получении mcp_request

---

## 7. Реализация: bridge.js

**Файл:** `Bridge/bridge.js` + `Bridge/package.json`
...
- [ ] Стриминг "по буквам" в Lyra-Chat.epf (сейчас показывается только финальный result)
- [ ] Интеграция полных промптов (router, analyst) в bridge.js v2

### Снятые вопросы (дополнительно)
- ~~Обработка 1С (WebSocket-клиент + чат-интерфейс)~~ — **реализовано**: Lyra-Chat.epf (2026-02-16). Внешняя обработка на базе ЕХТ_Чат с WebSocket-клиентом, HTML-чатом, Markdown, MCP-инструментами
...
| Файл | Что содержит |
|---|---|
| `Bridge/bridge.js` | **v2 bridge** — WebSocket-сервер + MCP-режим (Claude Code CLI) |
| `Bridge/lyra-bridge-sdk.js` | **v3 bridge** — Agent SDK edition (Haiku + Sonnet) |
| `Bridge/package.json` | Зависимости (ws, @modelcontextprotocol/sdk, claude-agent-sdk, zod) |
| `Bridge/emulator-1c.js` | Эмулятор 1С-клиента (тесты) |
| `Chat/src/Lyra-Chat/` | **Lyra-Chat.epf** — внешняя обработка 1С (чат + WebSocket + MCP) |
| `lyra-architecture.md` | Архитектура проекта (целевая + текущая) |
| `lyra-router-prompt.md` | Системный промпт Haiku (роутер, 17.5 KB) |
| `lyra-analyst-prompt.md` | Системный промпт Sonnet (аналитик 1С) |
| `Bridge/PROTOCOL-V3.md` | Документация протокола v3 (Agent SDK) |
| `Bridge/BACKLOG.md` | Бэклог открытых вопросов |
