---
created: 2026-03-07T16:09:06 (UTC +07:00)
...
source: https://centrifugal.dev/docs/getting-started/design
author: 
---

# Design overview | Centrifugo

> ## Excerpt
> Centrifugo architecture and design overview. Covers idiomatic usage patterns, message delivery model, history considerations, and scalability guidelines.

---
Let's discuss some architectural and design topics about Centrifugo.

## Idiomatic usage[](https://centrifugal.dev/docs/getting-started/design#idiomatic-usage "Direct link to Idiomatic usage")
...
All requests from users that generate new data should first go to the application backend – i.e., calling the app backend API from the client side. The backend can validate the message, process it, save it into a database for long-term persistence – and then publish an event to a channel using [Centrifugo server API](https://centrifugal.dev/docs/server/server_api). This event is then efficiently broadcast by Centrifugo to all active channel subscribers.
...
![diagram_unidirectional_publish](https://centrifugal.dev/assets/images/design_3-04b0059ce37137e9c0a61181bd676b76.png)
...
![diagram_unidirectional_publish](https://centrifugal.dev/assets/images/design_2-67dd1af26ef2255270e61052d09085f9.png)
...
It's also possible to utilize Centrifugo's bidirectional connection for sending requests to the backend. To achieve this, Centrifugo provides [event proxy features](https://centrifugal.dev/docs/server/proxy). It's possible to send RPC (with a custom request-response) requests from the client to Centrifugo and the request will then be proxied to the application backend (see [RPC proxy](https://centrifugal.dev/docs/server/proxy#client-rpc-proxy)). Moreover, the proxy provides a way to utilize the bidirectional connection for publishing into channels (using [publish proxy](https://centrifugal.dev/docs/server/proxy#publish-proxy)). But again – in most real scenarios, your backend must validate the publication attempt, so the scheme will look like this:

![client generates content](https://centrifugal.dev/assets/images/design_1-2ea0c767a2aaac987f0b7b8fab6fe531.png)

## Message history considerations[](https://centrifugal.dev/docs/getting-started/design#message-history-considerations "Direct link to Message history considerations")
...
## Message delivery model[](https://centrifugal.dev/docs/getting-started/design#message-delivery-model "Direct link to Message delivery model")
...
## Message order guarantees[](https://centrifugal.dev/docs/getting-started/design#message-order-guarantees "Direct link to Message order guarantees")

Centrifugo [server publish API](https://centrifugal.dev/docs/server/server_api#publish) guarantees message order within a channel only when messages are published sequentially or within a single request. If messages are published in parallel, ordering is not guaranteed due to concurrent processing.
...
When using [asynchronous consumers](https://centrifugal.dev/docs/server/consumers), ordering also depends on the consumer implementation. For example, Kafka can preserve order within partitions, while other systems may not guarantee any ordering. Refer to the particular consumer description.

## Graceful degradation[](https://centrifugal.dev/docs/getting-started/design#graceful-degradation "Direct link to Graceful degradation")
...
## Online presence considerations[](https://centrifugal.dev/docs/getting-started/design#online-presence-considerations "Direct link to Online presence considerations")
...
## Scalability considerations[](https://centrifugal.dev/docs/getting-started/design#scalability-considerations "Direct link to Scalability considerations")

Centrifugo can scale horizontally with built-in Redis engine or with the Nats broker. See [engines](https://centrifugal.dev/docs/server/engines).
