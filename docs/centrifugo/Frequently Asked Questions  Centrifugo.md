---
created: 2026-03-06T18:11:46 (UTC +07:00)
...
source: https://centrifugal.dev/docs/faq
author: 
---

# Frequently Asked Questions | Centrifugo

> ## Excerpt
> Frequently asked questions about Centrifugo: connection limits, scaling with Redis, memory usage, channel best practices, message delivery, and troubleshooting.

---
Answers to popular questions here.

### Disconnected due to client credentials not found[](https://centrifugal.dev/docs/faq#disconnected-due-to-client-credentials-not-found "Direct link to Disconnected due to client credentials not found")
...
-   [JWT authentication](https://centrifugal.dev/docs/server/authentication)
-   [Connect proxy](https://centrifugal.dev/docs/server/proxy#connect-proxy) authentication
-   Using proxy to set [user authentication header](https://centrifugal.dev/docs/server/configuration#clientuser_id_http_header)

You can also [configure access without token](https://centrifugal.dev/docs/server/configuration#clientallow_anonymous_connect_without_token) – in this case Centrifugo will consider a connection without provided token anonymous. Or, if you just want to quickly experiment with Centrifugo during development, it's possible to turn on [client.insecure](https://centrifugal.dev/docs/server/configuration#clientinsecure) option – but it **should never be used in production** since disables most of security checks.

Another possible reason of first time connection problems - not properly configured [allowed\_origins](https://centrifugal.dev/docs/server/configuration#clientallowed_origins). Centrifugo server logs should also clearly indicate such issues on INFO level.

### How many connections can one Centrifugo instance handle?[](https://centrifugal.dev/docs/faq#how-many-connections-can-one-centrifugo-instance-handle "Direct link to How many connections can one Centrifugo instance handle?")
...
You can find a description of a test stand with million WebSocket connections in [this blog post](https://centrifugal.dev/blog/2020/02/10/million-connections-with-centrifugo). Though the point above is still valid – measure and [monitor](https://centrifugal.dev/docs/server/monitoring) your setup.

### Memory usage per connection?[](https://centrifugal.dev/docs/faq#memory-usage-per-connection "Direct link to Memory usage per connection?")
...
### Can Centrifugo scale horizontally?[](https://centrifugal.dev/docs/faq#can-centrifugo-scale-horizontally "Direct link to Can Centrifugo scale horizontally?")
...
See [engines](https://centrifugal.dev/docs/server/engines) and [scalability considerations](https://centrifugal.dev/docs/getting-started/design#scalability-considerations).

### Message delivery model[](https://centrifugal.dev/docs/faq#message-delivery-model "Direct link to Message delivery model")

See [design overview](https://centrifugal.dev/docs/getting-started/design#message-delivery-model)

### Message order guarantees[](https://centrifugal.dev/docs/faq#message-order-guarantees "Direct link to Message order guarantees")

See [design overview](https://centrifugal.dev/docs/getting-started/design#message-order-guarantees).

### Should I create channels explicitly?[](https://centrifugal.dev/docs/faq#should-i-create-channels-explicitly "Direct link to Should I create channels explicitly?")
...
### What about best practices with the number of channels?[](https://centrifugal.dev/docs/faq#what-about-best-practices-with-the-number-of-channels "Direct link to What about best practices with the number of channels?")
...
### Any way to exclude message publisher from receiving a message from a channel?[](https://centrifugal.dev/docs/faq#any-way-to-exclude-message-publisher-from-receiving-a-message-from-a-channel "Direct link to Any way to exclude message publisher from receiving a message from a channel?")
...
### Can I have both binary and JSON clients in one channel?[](https://centrifugal.dev/docs/faq#can-i-have-both-binary-and-json-clients-in-one-channel "Direct link to Can I have both binary and JSON clients in one channel?")
...
### Online presence for chat apps - online status of your contacts[](https://centrifugal.dev/docs/faq#online-presence-for-chat-apps---online-status-of-your-contacts "Direct link to Online presence for chat apps - online status of your contacts")
...
You can solve this using a separate service that tracks the online status of your users (for example in Redis) and has a bulk API that returns online status approximation for a list of users. This way you will have an efficient scalable way to deal with online statuses. This is also available as a [Centrifugo PRO feature](https://centrifugal.dev/docs/pro/user_status).

### Centrifugo stops accepting new connections, why?[](https://centrifugal.dev/docs/faq#centrifugo-stops-accepting-new-connections-why "Direct link to Centrifugo stops accepting new connections, why?")

The most popular reason behind this is reaching the open file limit. You can make it higher, we described how to do this [nearby in this doc](https://centrifugal.dev/docs/server/infra_tuning). Also, check out [an article in our blog](https://centrifugal.dev/blog/2020/11/12/scaling-websocket) which mentions possible problems when dealing with many persistent connections like WebSocket.

### Can I use Centrifugo without reverse-proxy like Nginx before it?[](https://centrifugal.dev/docs/faq#can-i-use-centrifugo-without-reverse-proxy-like-nginx-before-it "Direct link to Can I use Centrifugo without reverse-proxy like Nginx before it?")
...
### Does Centrifugo work with HTTP/2?[](https://centrifugal.dev/docs/faq#does-centrifugo-work-with-http2 "Direct link to Does Centrifugo work with HTTP/2?")
...
### Does Centrifugo work with HTTP/3?[](https://centrifugal.dev/docs/faq#does-centrifugo-work-with-http3 "Direct link to Does Centrifugo work with HTTP/3?")
...
HTTP/3 does not work with ACME autocert TLS at the moment - i.e. you need to explicitly provide paths to cert and key files [as described here](https://centrifugal.dev/docs/server/tls#using-crt-and-key-files).

### Is there a way to use a single connection to Centrifugo from different browser tabs?[](https://centrifugal.dev/docs/faq#is-there-a-way-to-use-a-single-connection-to-centrifugo-from-different-browser-tabs "Direct link to Is there a way to use a single connection to Centrifugo from different browser tabs?")
...
### What if I need to send push notifications to mobile or web applications?[](https://centrifugal.dev/docs/faq#what-if-i-need-to-send-push-notifications-to-mobile-or-web-applications "Direct link to What if I need to send push notifications to mobile or web applications?")

We provide a [push notifications API](https://centrifugal.dev/docs/pro/push_notifications) implementation as part of Centrifugo PRO. It allows sending push notifications to devices - to Apple iOS devices via APNS, Android/iOS/Web devices via FCM. Also, Centrifugo PRO covers HMS (Huawei Mobile Services). But in general the task of push notification delivery may be done using another open-source solution, or with Firebase directly.
...
### How can I know a message is delivered to a client?[](https://centrifugal.dev/docs/faq#how-can-i-know-a-message-is-delivered-to-a-client "Direct link to How can I know a message is delivered to a client?")
...
### Can I publish new messages over a WebSocket connection from a client?[](https://centrifugal.dev/docs/faq#can-i-publish-new-messages-over-a-websocket-connection-from-a-client "Direct link to Can I publish new messages over a WebSocket connection from a client?")
...
### How to create a secure channel for two users only (private chat case)?[](https://centrifugal.dev/docs/faq#how-to-create-a-secure-channel-for-two-users-only-private-chat-case "Direct link to How to create a secure channel for two users only (private chat case)?")

There are several ways to achieve it:

-   use a private channel (starting with `$`) - every time a user subscribes to it your backend should provide a sign to confirm that subscription request. Read more in [channels chapter](https://centrifugal.dev/docs/server/channels#private-channel-prefix-)
-   next is [user limited channels](https://centrifugal.dev/docs/server/channels#user-channel-boundary-) (with `#`) - you can create a channel with a name like `dialog#42,567` to limit subscribers only to the user with id `42` and user with ID `567`, this does not fit well for channels with many or dynamic possible subscribers
-   you can use subscribe proxy feature to validate subscriptions, see [chapter about proxy](https://centrifugal.dev/docs/server/proxy)
...
### What's the best way to organize channel configuration?[](https://centrifugal.dev/docs/faq#whats-the-best-way-to-organize-channel-configuration "Direct link to What's the best way to organize channel configuration?")
...
### Does Centrifugo support webhooks?[](https://centrifugal.dev/docs/faq#does-centrifugo-support-webhooks "Direct link to Does Centrifugo support webhooks?")

[Proxy feature](https://centrifugal.dev/docs/server/proxy) allows integrating Centrifugo with your session mechanism (via connect proxy) and provides a way to react to connection events (rpc, subscribe, publish). Also, it opens a road for bidirectional communication with RPC calls. And periodic connection refresh hooks are also there.

Centrifugo does not support unsubscribe/disconnect hooks – see the reasoning below.

### Why Centrifugo does not have disconnect hooks?[](https://centrifugal.dev/docs/faq#why-centrifugo-does-not-have-disconnect-hooks "Direct link to Why Centrifugo does not have disconnect hooks?")
...
### Is it possible to listen to join/leave events on the app backend side?[](https://centrifugal.dev/docs/faq#is-it-possible-to-listen-to-joinleave-events-on-the-app-backend-side "Direct link to Is it possible to listen to join/leave events on the app backend side?")

No, join/leave events are only available in the client protocol. In most cases join event can be handled by using [subscribe proxy](https://centrifugal.dev/docs/server/proxy#subscribe-proxy). Leave events are harder – there is no unsubscribe hook available (mostly for the same reasons as for disconnect hook described above). So the workaround here can be similar to one for disconnect – ping an app backend periodically while client is subscribed and thus know that client is currently in a channel with some approximation in time.

### How scalable is the online presence and join/leave features?[](https://centrifugal.dev/docs/faq#how-scalable-is-the-online-presence-and-joinleave-features "Direct link to How scalable is the online presence and join/leave features?")
...
You may consider using a separate service to deal with presence status information that provides information in near real-time maybe with some reasonable approximation. Centrifugo PRO provides a [user status](https://centrifugal.dev/docs/pro/user_status) feature which may fit your needs.
...
### How to send initial data to channel subscriber?[](https://centrifugal.dev/docs/faq#how-to-send-initial-data-to-channel-subscriber "Direct link to How to send initial data to channel subscriber?")

Sometimes you need to send some initial state to a channel subscriber. Centrifugo provides a way to attach any data to a successful subscribe reply when using [subscribe proxy](https://centrifugal.dev/docs/server/proxy#subscribe-proxy) feature. See `data` and `b64data` fields. This data will be part of `subscribed` event context. And of course, you can always simply send request to get initial data from the application backend before or after subscribing to a channel without Centrifugo connection involved (i.e. using sth like general AJAX/HTTP call or passing data to the template when rendering an application page).

### Does Centrifugo support multitenancy?[](https://centrifugal.dev/docs/faq#does-centrifugo-support-multitenancy "Direct link to Does Centrifugo support multitenancy?")
...
### Is Centrifugo FIPS compliant?[](https://centrifugal.dev/docs/faq#is-centrifugo-fips-compliant "Direct link to Is Centrifugo FIPS compliant?")
...
-   When running with `GODEBUG=fips140=only`:
    ...
    -   Centrifugo PRO (starting from v6.5.0) automatically adapts in this mode to maintain full FIPS compliance by:
        ...
        -   disabling WebSocket over HTTP/1.1 in favor of WebSocket over HTTP/2, which does not require SHA-1 during the upgrade process. In this case, only connections that use [RFC 8441 – WebSocket over HTTP/2](https://centrifugal.dev/docs/transports/websocket#websocket-over-http2-rfc-8441) (must be explicitly enabled) will work. This can be a viable solution if you have a FIPS-approved load balancer with RFC 8441 support in front of Centrifugo PRO.
...
### I have not found an answer to my question here[](https://centrifugal.dev/docs/faq#i-have-not-found-an-answer-to-my-question-here "Direct link to I have not found an answer to my question here")
