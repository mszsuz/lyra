---
created: 2026-03-07T16:00:16 (UTC +07:00)
...
source: https://centrifugal.dev/docs/server/channels
author: 
---

# Channels and namespaces | Centrifugo

> ## Excerpt
> Centrifugo channels and namespaces configuration. Define channel behavior with options for presence, history, recovery, permissions, and more.
...
## What is a channel?[](https://centrifugal.dev/docs/server/channels#what-is-a-channel "Direct link to What is a channel?")
...
![pub_sub](https://centrifugal.dev/assets/images/pub_sub-5477abf6fb38219fc0848d9c9c3dc2b1.png)

A channel is simply a string - names like `news`, `comments`, `personal_feed` are examples of valid channel names. However, there are [predefined rules](https://centrifugal.dev/docs/server/channels#channel-name-rules) for these strings, as we will discuss later. You can define different behaviors for a channel using a range of available [channel options](https://centrifugal.dev/docs/server/channels#channel-options).
...
A channel can be part of a channel namespace. [Channel namespacing](https://centrifugal.dev/docs/server/channels#channel-namespaces) is a mechanism to define different behaviors for various channels within Centrifugo. Using namespaces is the recommended approach to manage channels – enabling only those channel options which are necessary for the specific real-time feature you are implementing with Centrifugo.

caution

Ensure you have defined a namespace in the configuration when using channel namespaces. Attempts to subscribe to a channel within an undefined namespace will result in [102: unknown channel](https://centrifugal.dev/docs/server/codes#unknown-channel) errors.

## Channel name rules[](https://centrifugal.dev/docs/server/channels#channel-name-rules "Direct link to Channel name rules")
...
-   `/` – for [Channel Patterns](https://centrifugal.dev/docs/pro/channel_patterns) in Centrifugo PRO
-   `*` – for the future Centrifugo needs
-   `&` – for the future Centrifugo needs

### namespace boundary (`:`)[](https://centrifugal.dev/docs/server/channels#namespace-boundary- "Direct link to namespace-boundary-")

`:` – is a channel namespace boundary. Namespaces are used to set custom options to a group of channels. Each channel belonging to the same namespace will have the same channel options. Read more about [namespaces](https://centrifugal.dev/docs/server/channels#channel-namespaces) and [channel options](https://centrifugal.dev/docs/server/channels#channel-options) below.
...
### user channel boundary (`#`)[](https://centrifugal.dev/docs/server/channels#user-channel-boundary- "Direct link to user-channel-boundary-")

The `#` symbol serves as the user channel boundary. It acts as a separator to create personal channels for users, known as _user-limited channels_, without requiring a subscription token.
...
User-limited channels must be enabled for a channel namespace using [allow\_user\_limited\_channels](https://centrifugal.dev/docs/server/channels#allow_user_limited_channels) option. See below more information about channel options and channel namespaces.

### private channel prefix (`$`)[](https://centrifugal.dev/docs/server/channels#private-channel-prefix- "Direct link to private-channel-prefix-")
...
### Channel is just a string[](https://centrifugal.dev/docs/server/channels#channel-is-just-a-string "Direct link to Channel is just a string")
...
## Channel namespaces[](https://centrifugal.dev/docs/server/channels#channel-namespaces "Direct link to Channel namespaces")
...
Namespace has a name, and can contain all the [channel options](https://centrifugal.dev/docs/server/channels#channel-options). Namespace `name` is required to be set. Name of namespace must be unique, must consist of letters, numbers, underscores, or hyphens and be more than 2 symbols length i.e. satisfy regexp `^[-a-zA-Z0-9_]{2,}$`.
...
```json
{  "channel": {    "without_namespace": {      "presence": true,      "history_size": 10,      "history_ttl": "30s"    },    "namespaces": [      {        "name": "facts",        "history_size": 10,        "history_ttl": "300s"      },      {        "name": "gossips"      }    ]  }}
```
...
## Channel options[](https://centrifugal.dev/docs/server/channels#channel-options "Direct link to Channel options")
...
### presence[](https://centrifugal.dev/docs/server/channels#presence "Direct link to presence")
...
See more details about [online presence design](https://centrifugal.dev/docs/getting-started/design#online-presence-considerations).

### join\_leave[](https://centrifugal.dev/docs/server/channels#join_leave "Direct link to join_leave")
...
### force\_push\_join\_leave[](https://centrifugal.dev/docs/server/channels#force_push_join_leave "Direct link to force_push_join_leave")
...
### history\_size[](https://centrifugal.dev/docs/server/channels#history_size "Direct link to history_size")
...
### history\_ttl[](https://centrifugal.dev/docs/server/channels#history_ttl "Direct link to history_ttl")

`history_ttl` ([duration](https://centrifugal.dev/docs/server/configuration#duration-type), default `0s`) – interval how long to keep channel history messages (with seconds precision).
...
Also note, that `history_ttl` must be less than [history\_meta\_ttl](https://centrifugal.dev/docs/server/channels#history_meta_ttl).
...
```bash
{  ...  "channel": {    "without_namespace": {      "history_size": 10,      "history_ttl": "60s"    }  }}
```
...
```css
curl --header "Content-Type: application/json" \  --header "X-API-Key: <API_KEY>" \  --request POST \  --data '{"channel": "chat:index", "limit": 100}' \  http://localhost:8000/api/history{    "result": {        "publications": [            {                "data": {                    "input": "1"                },                "offset": 1            },            {                "data": {                    "input": "2"                },                "offset": 2            }        ],        "epoch": "gWuY",        "offset": 2    }}
```

To call the history API from the client side, the client must have the necessary permissions. For more details, see the [history permission model](https://centrifugal.dev/docs/server/channel_permissions#history-permission-model).

See additional information about offsets and epoch in [History and recovery](https://centrifugal.dev/docs/server/history_and_recovery) chapter.
...
### history\_meta\_ttl[](https://centrifugal.dev/docs/server/channels#history_meta_ttl "Direct link to history_meta_ttl")

`history_meta_ttl` ([duration](https://centrifugal.dev/docs/server/configuration#duration-type)) – sets a time of history stream metadata expiration (with seconds precision).

If not specified Centrifugo namespace inherits value from `global_history_meta_ttl` ([duration](https://centrifugal.dev/docs/server/configuration#duration-type)) option which is 30 days by default (`"720h"`). This should be a good default for most use cases to avoid tweaking `history_meta_ttl` on a namespace level at all. If you have `history_ttl` greater than 30 days – then increase `history_meta_ttl` for namespace (recommended) or increase `global_history_meta_ttl` to be larger than `history_ttl`.
...
### force\_positioning[](https://centrifugal.dev/docs/server/channels#force_positioning "Direct link to force_positioning")
...
### force\_recovery[](https://centrifugal.dev/docs/server/channels#force_recovery "Direct link to force_recovery")
...
### allow\_subscribe\_for\_anonymous[](https://centrifugal.dev/docs/server/channels#allow_subscribe_for_anonymous "Direct link to allow_subscribe_for_anonymous")
...
### allow\_publish\_for\_subscriber[](https://centrifugal.dev/docs/server/channels#allow_publish_for_subscriber "Direct link to allow_publish_for_subscriber")
...
### allow\_publish\_for\_client[](https://centrifugal.dev/docs/server/channels#allow_publish_for_client "Direct link to allow_publish_for_client")
...
### allow\_publish\_for\_anonymous[](https://centrifugal.dev/docs/server/channels#allow_publish_for_anonymous "Direct link to allow_publish_for_anonymous")
...
### allow\_history\_for\_subscriber[](https://centrifugal.dev/docs/server/channels#allow_history_for_subscriber "Direct link to allow_history_for_subscriber")
...
### allow\_history\_for\_client[](https://centrifugal.dev/docs/server/channels#allow_history_for_client "Direct link to allow_history_for_client")

`allow_history_for_client` (boolean, default `false`) – allows all clients to call history information in a namespace.

### allow\_history\_for\_anonymous[](https://centrifugal.dev/docs/server/channels#allow_history_for_anonymous "Direct link to allow_history_for_anonymous")
...
### allow\_presence\_for\_subscriber[](https://centrifugal.dev/docs/server/channels#allow_presence_for_subscriber "Direct link to allow_presence_for_subscriber")
...
### allow\_presence\_for\_client[](https://centrifugal.dev/docs/server/channels#allow_presence_for_client "Direct link to allow_presence_for_client")

`allow_presence_for_client` (boolean, default `false`) – allows all clients to call presence information in a namespace.

### allow\_presence\_for\_anonymous[](https://centrifugal.dev/docs/server/channels#allow_presence_for_anonymous "Direct link to allow_presence_for_anonymous")
...
### allow\_user\_limited\_channels[](https://centrifugal.dev/docs/server/channels#allow_user_limited_channels "Direct link to allow_user_limited_channels")
...
### channel\_regex[](https://centrifugal.dev/docs/server/channels#channel_regex "Direct link to channel_regex")
...
```json
{  "channel": {    "namespaces": [      {        "name": "chat",        "allow_subscribe_for_client": true,        "channel_regex": "^[\d+]$"      }    ]  }}
```
...
### delta\_publish[](https://centrifugal.dev/docs/server/channels#delta_publish "Direct link to delta_publish")
...
### allowed\_delta\_types[](https://centrifugal.dev/docs/server/channels#allowed_delta_types "Direct link to allowed_delta_types")
...
### publication\_data\_format[](https://centrifugal.dev/docs/server/channels#publication_data_format "Direct link to publication_data_format")
...
```json
{  "channel": {    "publication_data_format": "json",    "without_namespace": {      "history_size": 10,      "history_ttl": "60s"    }  }}
```

Example configuration with namespace-specific format:

```json
{  "channel": {    "namespaces": [      {        "name": "json_only",        "publication_data_format": "json",        "history_size": 10,        "history_ttl": "60s"      }    ]  }}
```

Example configuration with global format and namespace override:

```python
{  "channel": {    "publication_data_format": "json",    "namespaces": [      {        "name": "binary_data",        "publication_data_format": "binary"      },      {        "name": "json_data"        // Inherits global "json" format      }    ]  }}
```
...
### allow\_tags\_filter[](https://centrifugal.dev/docs/server/channels#allow_tags_filter "Direct link to allow_tags_filter")

Available since Centrifugo v6.4.0.

`allow_tags_filter` (boolean, default `false`) - allows using tags filter when subscribing to channels in a namespace. See [Channel publication filtering](https://centrifugal.dev/docs/server/publication_filtering) chapter for more details.

### subscribe\_proxy\_enabled[](https://centrifugal.dev/docs/server/channels#subscribe_proxy_enabled "Direct link to subscribe_proxy_enabled")

`subscribe_proxy_enabled` (boolean, default `false`) – turns on subscribe proxy, more info in [proxy chapter](https://centrifugal.dev/docs/server/proxy)

### subscribe\_proxy\_name[](https://centrifugal.dev/docs/server/channels#subscribe_proxy_name "Direct link to subscribe_proxy_name")

`subscribe_proxy_name` (string, default `""`) – allows setting custom subscribe proxy to use by name. More info in [proxy chapter](https://centrifugal.dev/docs/server/proxy).

### publish\_proxy\_enabled[](https://centrifugal.dev/docs/server/channels#publish_proxy_enabled "Direct link to publish_proxy_enabled")

`publish_proxy_enabled` (boolean, default `false`) – turns on publish proxy, more info in [proxy chapter](https://centrifugal.dev/docs/server/proxy).

### publish\_proxy\_name[](https://centrifugal.dev/docs/server/channels#publish_proxy_name "Direct link to publish_proxy_name")

`publish_proxy_name` (string, default `""`) – allows setting custom publish proxy to use by name. More info in [proxy chapter](https://centrifugal.dev/docs/server/proxy).

### sub\_refresh\_proxy\_enabled[](https://centrifugal.dev/docs/server/channels#sub_refresh_proxy_enabled "Direct link to sub_refresh_proxy_enabled")

`sub_refresh_proxy_enabled` (boolean, default `false`) – turns on sub refresh proxy, more info in [proxy chapter](https://centrifugal.dev/docs/server/proxy).

### sub\_refresh\_proxy\_name[](https://centrifugal.dev/docs/server/channels#sub_refresh_proxy_name "Direct link to sub_refresh_proxy_name")

`sub_refresh_proxy_name` (string, default `""`) – allows setting custom sub refresh proxy to use by name. More info in [proxy chapter](https://centrifugal.dev/docs/server/proxy).

### subscribe\_stream\_proxy\_enabled[](https://centrifugal.dev/docs/server/channels#subscribe_stream_proxy_enabled "Direct link to subscribe_stream_proxy_enabled")

`subscribe_stream_proxy_enabled` (boolean, default `false`) - turns on subscribe stream proxy, see [subscription streams](https://centrifugal.dev/docs/server/proxy_streams).

### subscribe\_stream\_proxy\_name[](https://centrifugal.dev/docs/server/channels#subscribe_stream_proxy_name "Direct link to subscribe_stream_proxy_name")

`subscribe_stream_proxy_name` (string, default `""`) – allows setting custom subscribe stream proxy to use by name. See [subscription streams](https://centrifugal.dev/docs/server/proxy_streams).

### subscribe\_stream\_proxy\_bidirectional[](https://centrifugal.dev/docs/server/channels#subscribe_stream_proxy_bidirectional "Direct link to subscribe_stream_proxy_bidirectional")

`subscribe_stream_proxy_bidirectional` (boolean, default `false`) – allows using bidirectional subscribe stream. See [subscription streams](https://centrifugal.dev/docs/server/proxy_streams).

### cache\_empty\_proxy\_enabled[](https://centrifugal.dev/docs/server/channels#cache_empty_proxy_enabled "Direct link to cache_empty_proxy_enabled")

`cache_empty_proxy_enabled` (boolean, default `false`, Centrifugo PRO only) – turns on [cache empty proxy](https://centrifugal.dev/docs/pro/channel_cache_empty).

### cache\_empty\_proxy\_name[](https://centrifugal.dev/docs/server/channels#cache_empty_proxy_name "Direct link to cache_empty_proxy_name")

`cache_empty_proxy_name` (string, default `""`, Centrifugo PRO only) – allows setting custom cache empty proxy to use by name.

### state\_proxy\_enabled[](https://centrifugal.dev/docs/server/channels#state_proxy_enabled "Direct link to state_proxy_enabled")

`state_proxy_enabled` (boolean, default `false`, Centrifugo PRO only) - allows enabling [channel state proxy](https://centrifugal.dev/docs/pro/channel_state_events)

### state\_events[](https://centrifugal.dev/docs/server/channels#state_events "Direct link to state_events")

`state_events` (array of strings, empty by default, Centrifugo PRO only) - can help configuring notifications about channel's `occupied` and `vacated` state. See [more details](https://centrifugal.dev/docs/pro/channel_state_events) in Centrifugo PRO docs.

### shared\_position\_sync[](https://centrifugal.dev/docs/server/channels#shared_position_sync "Direct link to shared_position_sync")

`shared_position_sync` (boolean, default `false`, Centrifugo PRO only) - can help reducing the number of position synchronization requests from Centrifugo to Broker's history API, see [more details](https://centrifugal.dev/docs/pro/scalability#shared-position-sync) in Centrifugo PRO docs.

### subscribe\_cel[](https://centrifugal.dev/docs/server/channels#subscribe_cel "Direct link to subscribe_cel")

`subscribe_cel` (string, default `""`, Centrifugo PRO only) – CEL expression for subscribe permission, see more details in [Channel CEL expressions](https://centrifugal.dev/docs/pro/cel_expressions) of Centrifugo PRO.

### publish\_cel[](https://centrifugal.dev/docs/server/channels#publish_cel "Direct link to publish_cel")

`publish_cel` (string, default `""`, Centrifugo PRO only) – CEL expression for publish permission, see more details in [Channel CEL expressions](https://centrifugal.dev/docs/pro/cel_expressions) of Centrifugo PRO.

### history\_cel[](https://centrifugal.dev/docs/server/channels#history_cel "Direct link to history_cel")

`history_cel` (string, default `""`, Centrifugo PRO only) – CEL expression for history permission, see more details in [Channel CEL expressions](https://centrifugal.dev/docs/pro/cel_expressions) of Centrifugo PRO.

### presence\_cel[](https://centrifugal.dev/docs/server/channels#presence_cel "Direct link to presence_cel")

`presence_cel` (string, default `""`, Centrifugo PRO only) – CEL expression for presence permission, see more details in [Channel CEL expressions](https://centrifugal.dev/docs/pro/cel_expressions) of Centrifugo PRO.

### batch\_max\_size[](https://centrifugal.dev/docs/server/channels#batch_max_size "Direct link to batch_max_size")

`batch_max_size` (integer, default `0`) – maximum batch size when using per-channel batching. See more details in Centrifugo PRO [Message batching control](https://centrifugal.dev/docs/pro/client_message_batching) documentation.

### batch\_max\_delay[](https://centrifugal.dev/docs/server/channels#batch_max_delay "Direct link to batch_max_delay")

`batch_max_delay` ([duration](https://centrifugal.dev/docs/server/configuration#duration-type), default `0s`) – maximum delay time when using per-channel batching. See more details in Centrifugo PRO [Message batching control](https://centrifugal.dev/docs/pro/client_message_batching) documentation.

### batch\_flush\_latest[](https://centrifugal.dev/docs/server/channels#batch_flush_latest "Direct link to batch_flush_latest")

`batch_flush_latest` (bool, default `false`) – allows sending only the latest message in collected batch. See more details in Centrifugo PRO [Message batching control](https://centrifugal.dev/docs/pro/client_message_batching) documentation.

### allow\_channel\_compaction[](https://centrifugal.dev/docs/server/channels#allow_channel_compaction "Direct link to allow_channel_compaction")

Available since Centrifugo v6.4.0.

`allow_channel_compaction` (boolean, default `false`, Centrifugo PRO only) – allows real-time SDKs to negotiate [channel compaction](https://centrifugal.dev/docs/pro/bandwidth_optimizations#channel-compaction) in a namespace.

## Channel config examples[](https://centrifugal.dev/docs/server/channels#channel-config-examples "Direct link to Channel config examples")
