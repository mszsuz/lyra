---
created: 2026-03-06T18:09:47 (UTC +07:00)
...
source: https://centrifugal.dev/docs/getting-started/quickstart
author: 
---

# Quickstart tutorial ⏱️ | Centrifugo

> ## Excerpt
> Build your first real-time app with Centrifugo. Step-by-step tutorial covering WebSocket connections, channel subscriptions, and publishing messages.
...
First, you need to [install Centrifugo](https://centrifugal.dev/docs/getting-started/installation). In this example, we are using a binary file release which is fine for development. Once you have the Centrifugo binary available on your machine, you can generate the minimal required configuration file with the following command:
...
```json
{  "client": {    "token": {      "hmac_secret_key": "bbe7d157-a253-4094-9759-06a8236543f9"    },    "allowed_origins": []  },  "http_api": {    "key": "d7627bb6-2292-4911-82e1-615c0ed3eebb"  },  "admin": {    "enabled": false,    "password": "d0683813-0916-4c49-979f-0e08a686b727",    "secret": "4e9eafcf-0120-4ddd-b668-8dc40072c78e"  }}
```
...
![Admin web panel](https://centrifugal.dev/assets/images/quick_start_admin_v5-52b31f2dfd00de4c5169e2b24c71bd1f.png)

Inside the admin panel, you should see that one Centrifugo node is running, and it does not have connected clients:

![Admin web panel](https://centrifugal.dev/assets/images/quick_start_logged_in_v4-e81864800a451fd7119a91f51c1b466a.png)
...
```bash
{  "client": {    ...    "allowed_origins": ["http://localhost:3000"]  },
```

Allowed origins is a security option for request originating from web browsers – see [more details](https://centrifugal.dev/docs/server/configuration#clientallowed_origins) in server configuration docs.
...
```sql
HMAC SHA-256 JWT for user "123722" with expiration TTL 168h0m0s:eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM3MjIiLCJleHAiOjE2NTU0NDgyOTl9.mUU9s5kj3yqp-SAEqloGy8QBgsLg0llA7lKUNwtHRnw
```

– you will have a different token value since this one is based on the randomly generated `client.token.hmac_secret_key` from the configuration file we created at the beginning of this tutorial. See the [token authentication docs](https://centrifugal.dev/docs/server/authentication) for information about proper token generation in a real application.
...
```cpp
const centrifuge = new Centrifuge("ws://localhost:8000/connection/websocket", {    token: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM3MjIiLCJleHAiOjE2NTU0NDgyOTl9.mUU9s5kj3yqp-SAEqloGy8QBgsLg0llA7lKUNwtHRnw"});
```
...
We need to give the client permission to subscribe to the channel `channel`. There are several ways to do this. For example, the client can provide a [subscription JWT](https://centrifugal.dev/docs/server/channel_token_auth) for a channel. But here we will use an option to allow all authenticated clients to subscribe to any channel.
...
```bash
{  ...  "channel": {    "without_namespace": {      "allow_subscribe_for_client": true    }  }}
```

tip

A good practice with Centrifugo is to configure [channel namespaces](https://centrifugal.dev/docs/server/channels#channel-namespaces) for the different types of real-time features you have in the application. By defining namespaces, you can achieve granular control over channel behavior and permissions. Here we use configuration for channels which do not start with a channel namespace name.
...
![Connected](https://centrifugal.dev/assets/images/quick_start_ws_frames_v4-9e299975b617ef20f961ed68692e67d9.png)

Note that in this example, we generated a connection JWT – but it has an expiration time, so after some time, Centrifugo will stop accepting those tokens. In real life, you need to add a token refresh function to the client to rotate tokens. See our [client API SDK spec](https://centrifugal.dev/docs/transports/client_api).

Go back to Admin UI - [http://localhost:8000/](http://localhost:8000/) - and make sure you see updated Centrifugo node stats – they should display one active client connection.
...
![Admin publish](https://centrifugal.dev/assets/images/quick_start_publish_v4-54ba7f45435ba110190f21e2c3e1dd24.png)
...
![Message received](https://centrifugal.dev/assets/images/quick_start_message_v4-81964ce5777a2c65910116a04fa6e371.png)

Let's also look at how you can publish data to a channel over Centrifugo server API from a terminal using `curl` tool:

```css
curl --header "Content-Type: application/json" \  --header "X-API-Key: d7627bb6-2292-4911-82e1-615c0ed3eebb" \  --request POST \  --data '{"channel": "channel", "data": {"value": 2}}' \  http://localhost:8000/api/publish
```

– where for `X-API-Key` header we set `api_key` value from Centrifugo config file generated above.

We did it! We built the simplest browser real-time app with Centrifugo and its JavaScript client. It doesn't have a backend; it's not very useful, to be honest, but it should give you an insight into how to start working with the Centrifugo server. Read more about the Centrifugo server in the next documentation chapters – it can do much more than we just showed here. The [Integration guide](https://centrifugal.dev/docs/getting-started/integration) describes the process of idiomatic Centrifugo integration with your application backend. And [chat/messenger app tutorial](https://centrifugal.dev/docs/tutorial/intro) provides a comprehensive guide how to build real-time app with Centrifugo from scratch.
