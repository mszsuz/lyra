...
| Поле | Тип | Назначение |
|------|-----|------------|
| SessionID (измерение) | Строка 36 | UUID сессии |
| FormID | Строка 36 | UUID формы обработки (для переподключения) |
| ConfigName | Строка 256 | Имя конфигурации клиента |
| ConfigVersion | Строка 50 | Версия конфигурации |
| ConfigID | Строка 36 | Идентификатор конфигурации |
| Computer | Строка 256 | Имя компьютера |
| ConnectionString | Строка 1024 | Строка подключения |
| Channel | Строка 100 | Канал сессии (session:\<id\>) |
| ClientID | Строка 36 | UUID соединения Centrifugo |
| Status | Строка 50 | Статус (awaiting_auth, active, closed) |
| UserID | Строка 36 | ID пользователя (после auth) |
| Created | ДатаВремя | Время создания |
| LastActivity | ДатаВремя | Последняя активность |

**Статус:** код написан и расширение загружено в базу Lyra-TEST. Блокер: после перезапуска автономного сервера 1С WebSocket-клиент Роутера не стартует автоматически — нужно принудительно вызвать `WebSocketКлиенты.НайтиПоКлючу("Роутер").Подключить()`.
