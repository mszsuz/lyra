...
| **MDM** | НСИ: Пользователи (привязка к device_id, email опционально) + подчинённый Базы (список баз пользователя) | Серверная база 1С |
...
Centrifugo v6.6.2 — сервер реального времени (WebSocket/SSE). Единый транспорт для всех компонентов. Порт: **11911**.

### Внешний доступ: Tuna TCP tunnel

Centrifugo слушает на `localhost:11911`. Для доступа извне используется **Tuna TCP tunnel** — статический TCP-проброс через `ru.tuna.am:35773`.

```
Интернет (EPF / мобильное) → ru.tuna.am:35773 → TCP tunnel → localhost:11911
```

Команда запуска: `tuna tcp 11911 --port=lyra`. Порт `lyra` (35773) зарезервирован в дашборде Tuna — статический, не меняется при перезапуске. Tunnel запускается как Windows-сервис.

**Почему TCP, а не HTTP tunnel:** HTTP-tunnel Tuna модифицирует заголовки WebSocket-ответа — меняет регистр `Sec-WebSocket-Accept` на `Sec-Websocket-Accept` (строчная `w`). Библиотека boost.beast в тонком клиенте 1С отклоняет такой handshake. TCP-tunnel пробрасывает сырые байты без модификации заголовков — handshake проходит корректно.

### connect.json — удалённая конфигурация подключения

Публичный файл в репозитории GitHub: `https://raw.githubusercontent.com/mszsuz/lyra/master/connect.json`

```json
{
  "ws": "ws://ru.tuna.am:35773/connection/websocket",
  "version": 1
}
```

Клиенты (EPF и мобильное) загружают `connect.json` при запуске и берут из него WebSocket URL. Это позволяет менять адрес Centrifugo (смена IP, порта, tunnel) без пересборки EPF и обновления мобильного приложения.

**Реализация в EPF:** `МодульСессия.ПолучитьАдресИзУдалённогоКонфига()` — серверная функция, `HTTPСоединение` к GitHub. При недоступности GitHub — fallback на зашитый URL.

**Реализация в мобильном:** `CentrifugoConfig.getWsUrl()` — `HttpClient`, in-memory кэш. При недоступности GitHub — fallback на зашитый URL.
...
- Регистрация автоматическая (device_id = user_id), без SMS/телефона
...
Регистрация **автоматическая** — при первом запуске (user_id отсутствует на устройстве). Без SMS, без телефона, без подтверждений. `device_id` = `user_id`.

```
Мобильное приложение:
  1. Генерирует device_id (UUID), сохраняет в secure storage
  2. connect к Centrifugo (общий mobile JWT с channels claim: ["mobile:lobby"])
  3. publish register в mobile:lobby:
     {type: "register", device_id: "uuid-устройства"}

Роутер (подписан на mobile:lobby):
  4. Получает register
  5. MDM: создать пользователя (device_id = user_id)
     -> Справочник Пользователи (владелец) + подчинённый справочник Базы
     -> device_id уже зарегистрирован: publish register_error
  6. Server API publish в mobile:lobby:
     {type: "register_ack", device_id: "...", status: "ok", user_id: "uuid"}

Мобильное приложение:
  7. Фильтрует register_ack по device_id (принимает только свой)
  8. Сохраняет user_id в secure storage (Keychain / Keystore)
  9. Отключается от mobile:lobby — готово к сканированию QR
```

Все ответы регистрации публикуются в общий канал `mobile:lobby`. Мобильное подписано через JWT channels claim (авто-подписка при connect). Фильтрация на клиенте по `device_id`.

**MDM: справочники**
- **Пользователи** -- владелец, привязан к device_id. Email опционально для восстановления
...
- Email добавляется в профиле мобильного приложения (с подтверждением по коду)
- На новом устройстве: ввод email → код на email → подтверждение → привязка нового device_id к user_id
- Без email — восстановление невозможно, создаётся новый аккаунт
...
```
Lyra-Chat.epf --> WebSocket --> Centrifugo (:11911) --> канал session:lobby
```
...
1. stdio-bridge (Модератор):
      --centrifugo-url "ws://localhost:11911/connection/websocket"
...
Регистрация автоматическая — при первом запуске. Фильтрация на клиенте по `device_id`.
...
{"type": "register_ack", "device_id": "uuid-устройства", "status": "ok", "user_id": "uuid-пользователя"}

// Роутер → mobile:lobby (ошибка)
{"type": "register_error", "device_id": "uuid-устройства", "reason": "already_registered"}
```
...
|---------------------|------------------|---------------------------------------|
| `already_registered`| register_error   | device_id уже зарегистрирован         |
...
## RAG-обогащение (rag.mjs)

Перед отправкой вопроса в основную модель Роутер обогащает его метаданными и документацией через RAG-прослойку:

1. **Flash-агент** (бесплатная модель, настраивается в `rag.model`) выбирает поисковые запросы через function calling (~1 сек)
2. **Параллельные MCP-вызовы** — Vega (метаданные) + mcp-1c-docs (документация) через `Promise.allSettled` (~1 сек)
3. **Программная обрезка** — без LLM, только фильтрация по категориям и лимит на количество
4. Результат вставляется в тег `<rag>` перед вопросом пользователя

RAG никогда не блокирует основной flow: при ошибке или таймауте вопрос уходит без `<rag>`. Все промежуточные вызовы (fetch + res.json) обёрнуты в `withTimeout()`.
...
**Баланс — единый источник (BalanceNotifier):** `StateNotifierProvider<BalanceNotifier, double>` — единственное место хранения баланса в мобильном. Обновляется из `auth_ack`, `balance_update`, сканера QR. Все экраны (Home, Session) реактивно подписаны через `ref.watch(balanceProvider)`. Синхронизируется с `SecureStorage` при каждом изменении.

**Синхронизация сессий:** При получении `sessions_list` от Роутера мобильное сравнивает серверный список с локальным хранилищем — удаляет мёртвые сессии, обновляет живые. Это предотвращает «призраков» (закрытые на компьютере сессии, оставшиеся в списке мобильного).
