...
2. Роутер передаёт сообщение Модератору
3. Модератор проверяет: блокировка / напоминание / ОК
4. Роутер запускает Адаптер (если не запущен) → передаёт вопрос в Claude
5. Claude стримит ответ → Адаптер → Роутер → Centrifugo → `session:<session_id>` → Чат
6. После каждого ответа Claude Биллинг списывает стоимость и отправляет `balance_update` с остатком в канал сессии

### Модератор (проверка входящих)

```
Сообщение пользователя ──► Модератор ──► Модель (быстрая, дешёвая — Haiku или локальная)
                                              │
