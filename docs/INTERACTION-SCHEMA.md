...
Чат переподключается к Centrifugo с `chat_jwt` (отключается от lobby). JWT содержит `channels` claim -- Centrifugo автоматически подписывает на канал сессии при connect (отдельный subscribe не нужен). QR-код = `mobile_jwt`. Оба JWT бессрочные (без exp).
...
Мобильное приложение сканирует QR-код, получает `mobile_jwt`. Подключается к Centrifugo с этим JWT -- авто-подписка на канал сессии через `channels` claim. Мобильное НИКОГДА не подключается к lobby.
...
### 10. Модератор проверяет входящее сообщение
...
### 11. Роутер запускает адаптер (если ещё не запущен для сессии)
...
### 12. Адаптер передает вопрос в Claude, стримит ответ
...
```
Claude --> Адаптер --> Роутер --> Centrifugo --> канал сессии
                                                     |
...
Claude: tool_use v8_query("ВЫБРАТЬ КПП ИЗ Справочник.Контрагенты WHERE ...")
    |
    v
Адаптер --> Роутер --> Centrifugo --> канал сессии --> Lyra-Chat.epf (база пользователя)
                                                          |
                                                          v
                                                    Выполнить запрос на сервере 1С
                                                          |
                                                          v
Lyra-Chat.epf --> Centrifugo --> канал сессии --> Роутер --> Адаптер --> Claude
...
| **ЕХТ_Лира_Роутер** | Бизнес-логика: авторизация, маршрутизация, генерация JWT, запуск адаптеров | Серверная база 1С |
