...
Служебный канал `service:events` -- шина событий между серверными компонентами. Каждый компонент публикует свои события, остальные подписываются на нужные. Например: Роутер публикует `event_router_auth_completed` после авторизации (шаг 7) → Биллинг подписан на `service:events` и автоматически подключается к каналу сессии.

Все серверные компоненты (Роутер, Биллинг) подключаются к Centrifugo как **WebSocket-клиенты** -- компоненты могут быть разнесены на разные серверы.
...
## Подключение: hello + QR-авторизация
...
| `config_id` | `DEADBEEF-0000-0000-0000-00000000F00D` | Идентификатор конфигурации. Одинаковый для всех баз с одной типовой конфигурацией + версией (если не снята с поддержки). Если config_id есть в таблице соответствия — конфигурация типовая. Если нет — снята с поддержки или доработана. Эта информация передаётся модели в системном промпте. Получается через `ЕХТ_КонфигурацияПовтИсп.ИдентификаторКонфигурации()` |
...
Клиент получает hello_ack на канале сессии:
...
Роутер получает auth на канале сессии.
...
Канал сессии остается на всю сессию -- переключения не происходит.
...
Недостаточно средств (сессия не закрывается -- при пополнении Биллинг публикует event_billing_balance_checked ok → Роутер отправит auth_ack ok автоматически):
...
```json
{
  ...
  "message": "К сожалению, на вашем счёте недостаточно средств. Пополните баланс в мобильном приложении, и мы сразу продолжим!"
}
```

Биллинг недоступен (таймаут):
...
Обновление баланса (Биллинг отправляет при каждом изменении):
...
### 11. Роутер запускает stdio-bridge (если ещё не запущен для сессии)
...
| Компонент | Роль | Где живет |
|-----------|------|-----------|
| **Lyra-Chat.epf** | UI чата, WebSocket-клиент, exec-команды | База пользователя |
| **Мобильное приложение** | Авторизация (QR), голосовой ввод, фото -- устройство ввода для Чата. Баланс + пополнение | Телефон пользователя |
| **Centrifugo** | WebSocket-транспорт, каналы (session:lobby, mobile:lobby, каналы сессий), переподключение | Сервер Lyra |
| **ЕХТ_Лира_Роутер** | Бизнес-логика: авторизация, маршрутизация, модерация, генерация JWT, запуск адаптеров | Серверная база 1С |
| **MDM** | НСИ: справочник Пользователи (по телефону) + подчинённый справочник Базы | Серверная база 1С |
| **Биллинг** | Баланс, тарификация, учет запросов. WebSocket-клиент Centrifugo -- подключается к каналам сессий после auth, отправляет баланс в реальном времени | Серверная база 1С |
